...
export const IndexPComponent = ({nombre, salario}) => {
    const navigation = useNavigation();
  return (
    <Box style={style.box}>
        <Box style={style.internalBox}>
          <Text style={style.txt}>Nombre: {nombre} </Text>
          <Text style={style.txt}>Salario ${salario}</Text>
        </Box> 
        <Box style={{...style.internalBox, marginLeft: 70}}>
          <Button onPress={() => navigation.navigate('CreatePuesto')} variant="ghost" colorScheme="black" leftIcon={<Icon name='create-outline' size={20} color='black' />} _text={{fontSize: 16, fontWeight: '600', color: 'black'}}>Editar</Button>
